...
void R_BuildWorldVBO( msurface_t *surf, int surfCount )
{
	...
	// 0 item is unused
	vbo->items = ri.Hunk_Alloc( ( numStaticSurfaces + 1 ) * sizeof( vbo_item_t ) );
	vbo->items_count = numStaticSurfaces;

	// last item will be used for run length termination
	vbo->items_queue = ri.Hunk_Alloc( ( numStaticSurfaces + 1 ) * sizeof( int ) );
	...
	// index buffer
	vbo->ibo_buffer = ri.Hunk_Alloc( ibo_size );	
	vbo->ibo_offset = 0;
	vbo->ibo_size = ibo_size;

	// soft index buffer
	vbo->soft_buffer = ri.Hunk_Alloc( ibo_size );
	vbo->soft_buffer_indexes = 0;

	// ibo runs buffer
	vbo->ibo_items = ri.Hunk_Alloc( ( (numStaticIndexes / MIN_IBO_RUN) + 1 ) * sizeof( ibo_item_t ) );
