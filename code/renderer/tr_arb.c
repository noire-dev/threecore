...
static char *ARB_BuildPostFXProgram( char *buf ) {
    ...
    if ( r_fx_filmic->value != 0.0 ) {
		s += sprintf( s, "TEMP innerhueShift; \n" );
		s += sprintf( s, "PARAM innerhueRotation = { 1.0, 1.0, 1.0, 1.0 }; \n" );

		s += sprintf( s, "ADD base.xyz, base, 0.05; \n" );
		
		s += sprintf( s, "DP3 innerhueShift.x, base, innerhueRotation; \n" );
		s += sprintf( s, "MUL innerhueShift.xyz, base, innerhueShift.x; \n" );
		s += sprintf( s, "ADD innerhueShift.xyz, innerhueShift, 0.02; \n" );
		s += sprintf( s, "MUL innerhueShift.xyz, innerhueShift, 1.1; \n" );
		s += sprintf( s, "MOV base, innerhueShift; \n" );
		
		s += sprintf( s, "TEMP innerblurTexel, innerblurredColor; \n" );
		s += sprintf( s, "PARAM innerblurOffsets = { 0.002, 0.0, 0.0, 0.0 }; \n" );
		s += sprintf( s, "PARAM innerambientLight = { -0.0050, 0.0075, -0.0050, 5.0 }; \n" );
		s += sprintf( s, "ADD innerblurTexel.xy, baseTexCoord, innerblurOffsets; \n" );
		s += sprintf( s, "TEX innerblurTexel.r, innerblurTexel, texture[1], 2D; \n" );
		s += sprintf( s, "SUB innerblurTexel.xy, baseTexCoord, innerblurOffsets; \n" );
		s += sprintf( s, "TEX innerblurTexel.g, innerblurTexel, texture[1], 2D; \n" );
		s += sprintf( s, "ADD innerblurredColor.z, innerblurTexel.r, base.r; \n" );
		s += sprintf( s, "ADD innerblurredColor.y, innerblurTexel.g, base.g; \n" );
		s += sprintf( s, "ADD innerblurredColor.x, innerblurTexel.b, base.b; \n" );
		s += sprintf( s, "MUL innerblurredColor.xyz, innerblurredColor, innerambientLight; \n" );
		s += sprintf( s, "ADD base.xyz, base, innerblurredColor; \n" );
		
		s += sprintf( s, "PARAM innersRGBs = { 0.2126, 0.7152, 0.0722, 1.0 }; \n" );
		s += sprintf( s, "TEMP innercolors; \n" );
		s += sprintf( s, "DP3 innercolors.xyz, base, innersRGBs; \n" );
		s += sprintf( s, "LRP base.xyz, -0.30, innercolors, base; \n" );		
    }
