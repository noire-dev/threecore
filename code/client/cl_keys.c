// Copyright (C) 1999-2005 ID Software, Inc.
// Copyright (C) 2023-2026 Noire.dev
// ThreeCore — GPLv2; see LICENSE for details.

#include "client.h"

static void CL_KeyDownEvent(int key) {
	if(key == K_ESCAPE) {
		if(Key_GetCatcher() & KEYCATCH_CGAME) {
			Key_SetCatcher(Key_GetCatcher() & ~KEYCATCH_CGAME);
			return;
		}

		if(!(Key_GetCatcher() & KEYCATCH_UI)) {
			VM_Call(uivm, 1, UI_SET_ACTIVE_MENU, UI_ENABLE);
			return;
		}

		VM_Call(uivm, 3, UI_KEY_EVENT, key, qtrue, qfalse);
		return;
	}

	if(Key_GetCatcher() & KEYCATCH_UI) {
		if(uivm) VM_Call(uivm, 3, UI_KEY_EVENT, key, qtrue, qfalse);
	} else {
		Key_ParseBinding(key, qtrue);
	}
}

static void CL_KeyUpEvent(int key) {
	if(cls.state != CA_DISCONNECTED) Key_ParseBinding(key, qfalse);
	if(Key_GetCatcher() & KEYCATCH_UI && uivm) VM_Call(uivm, 3, UI_KEY_EVENT, key, qfalse, qfalse);
}

void CL_KeyEvent(int key, qboolean down) {
	if(down) CL_KeyDownEvent(key);
	else CL_KeyUpEvent(key);
}

void CL_CharEvent(int key) {
	if(key == 127) return;
	if(Key_GetCatcher() & KEYCATCH_UI) VM_Call(uivm, 3, UI_KEY_EVENT, key, qtrue, qtrue);
}

void Key_ClearStates(void) {
	int i;

	for(i = 0; i < MAX_KEYS; i++) {
		if(keys[i].down) CL_KeyEvent(i, qfalse);

		keys[i].down = qfalse;
	}
}

static int keyCatchers = 0;

int Key_GetCatcher(void) { return keyCatchers; }

void Key_SetCatcher(int catcher) {
	if(catcher != keyCatchers) Key_ClearStates();
	keyCatchers = catcher;
}
