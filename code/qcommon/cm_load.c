...
static void CMod_LoadShaders( const lump_t *l ) {
	...
	cm.shaders = Z_Malloc( count * sizeof( *cm.shaders ) );
	cm.numShaders = count;
...
static void CMod_LoadSubmodels( const lump_t *l ) {
	...
	cm.cmodels = Z_Malloc( count * sizeof( *cm.cmodels ) );
	...
	for ( i=0 ; i<count ; i++, in++)
	{
		...
		indexes = Z_Malloc( out->leaf.numLeafBrushes * 4 );
		...
		indexes = Z_Malloc( out->leaf.numLeafSurfaces * 4 );
...
static void CMod_LoadNodes( const lump_t *l ) {
	...
	cm.nodes = Z_Malloc( count * sizeof( *cm.nodes ) );
	cm.numNodes = count;
...
static void CMod_LoadBrushes( const lump_t *l ) {
	...
	cm.brushes = Z_Malloc( ( BOX_BRUSHES + count ) * sizeof( *cm.brushes ) );
	cm.numBrushes = count;
...
static void CMod_LoadLeafs( const lump_t *l )
{
	...
	cm.leafs = Z_Malloc( ( BOX_LEAFS + count ) * sizeof( *cm.leafs ) );
	cm.numLeafs = count;
	...
	cm.areas = Z_Malloc( cm.numAreas * sizeof( *cm.areas ) );
	cm.areaPortals = Z_Malloc( cm.numAreas * cm.numAreas * sizeof( *cm.areaPortals ) );
}
...
static void CMod_LoadPlanes( const lump_t *l )
{
	...
	cm.planes = Z_Malloc( ( BOX_PLANES + count ) * sizeof( *cm.planes ) );
	cm.numPlanes = count;
...
static void CMod_LoadLeafBrushes( const lump_t *l )
{
	...
	cm.leafbrushes = Z_Malloc( (count + BOX_BRUSHES) * sizeof( *cm.leafbrushes ) );
...
static void CMod_LoadLeafSurfaces( const lump_t *l )
{
	...
	cm.leafsurfaces = Z_Malloc( count * sizeof( *cm.leafsurfaces ) );
...
static void CMod_LoadBrushSides( const lump_t *l )
{
	...
	cm.brushsides = Z_Malloc( ( BOX_SIDES + count ) * sizeof( *cm.brushsides ) );
...
static void CMod_LoadVisibility( const lump_t *l ) {
	...
	if ( !len ) {
		...
		cm.visibility = Z_Malloc( cm.clusterBytes );
...
static void CMod_LoadPatches( const lump_t *surfs, const lump_t *verts ) {
	...
	cm.surfaces = Z_Malloc( cm.numSurfaces * sizeof( cm.surfaces[0] ) );
	...
	for ( i = 0 ; i < count ; i++, in++ ) {
		...
		cm.surfaces[ i ] = patch = Z_Malloc( sizeof( *patch ) );
